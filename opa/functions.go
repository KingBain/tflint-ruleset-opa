...
func Functions(runner tflint.Runner) []func(*rego.Rego) {
	return []func(*rego.Rego){
		...
		exprMap(runner).asOption(),
		exprCall(runner).asOption(),
		issueFunc().asOption(),
	}
}
...
func TesterFunctions(runner tflint.Runner) []*tester.Builtin {
	return []*tester.Builtin{
		...
		exprMap(runner).asTester(),
		exprCall(runner).asTester(),
		issueFunc().asTester(),
	}
}
...
//
//	expr (raw_expr) static list expression. Typically this is the value of the attribute retrieved by the terraform functions.
//
// Returns:
//
//	exprs (array[raw_expr]) list of expressions
func exprList(runner tflint.Runner) *function1 {
	return &function1{
		...
		Func: func(_ rego.BuiltinContext, exprArg *ast.Term) (*ast.Term, error) {
			...
			for i, e := range exprs {
				ret[i] = rawExprToJSON(e, file.Bytes)
			}
...
//
// Extracts a list of key value pairs from the given static map expression.
// This is equivalent to hcl.ExprMap in hashicorp/hcl.
//
//	expr (raw_expr) static map expression. Typically this is the value of the attribute retrieved by the terraform functions.
//
// Returns:
//
//	pairs (array[key_value]) list of key value pairs
func exprMap(runner tflint.Runner) *function1 {
	return &function1{
		function: function{
			Decl: &rego.Function{
				Name:    "hcl.expr_map",
		...
		Func: func(_ rego.BuiltinContext, exprArg *ast.Term) (*ast.Term, error) {
			...
			for i, pair := range pairs {
				ret[i] = map[string]map[string]any{
					"key":   rawExprToJSON(pair.Key, file.Bytes),
					"value": rawExprToJSON(pair.Value, file.Bytes),
				}
			}
...
//
// Extracts the function name and arguments from the given function call expression.
// This is equivalent to hcl.ExprCall in hashicorp/hcl.
//
//	expr (raw_expr) function call expression. Typically this is the value of the attribute retrieved by the terraform functions.
//
// Returns:
//
//	call (call) function call object
func exprCall(runner tflint.Runner) *function1 {
	return &function1{
		function: function{
			Decl: &rego.Function{
				Name:    "hcl.expr_call",
		...
		Func: func(_ rego.BuiltinContext, exprArg *ast.Term) (*ast.Term, error) {
			...
			for i, arg := range call.Arguments {
				arguments[i] = rawExprToJSON(arg, file.Bytes)
			}
			ret := map[string]any{
				"name":       call.Name,
